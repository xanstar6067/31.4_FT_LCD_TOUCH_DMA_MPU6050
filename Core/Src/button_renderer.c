...
void ButtonRenderer_Draw(VirtualButton* button) {
    ...
    switch (button->type) {
        case BUTTON_TYPE_FILLED:
            DISPLAY_FillRectangle_DMA(button->x1, button->y1, width, height, button->fill_color);
            break;

        case BUTTON_TYPE_OUTLINED:
            DISPLAY_FillRectangle_DMA(button->x1, button->y1, width, height, button->fill_color);
            DISPLAY_DrawRect(button->outline_color, button->x1, button->y1, button->x2, button->y2);
            break;
        ...
        case BUTTON_TYPE_BITMAP:
            if (button->bitmap) {
                ...
                DISPLAY_DrawImage_DMA(button->x1, button->y1, width, height, button->bitmap);
                //DISPLAY_DrawImage_DMA(button->x1, button->y1, width, height, button->bitmap, 240);
